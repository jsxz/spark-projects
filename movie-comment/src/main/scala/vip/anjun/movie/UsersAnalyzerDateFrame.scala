...
object UsersAnalyzerDateFrame {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var dataPath = "/bd/workspace/spark-projects/movie-comment/src/main/resources/data/"
    if (args.length > 0) {
      masterUrl = args(0)
    ...
    val sparkConf = new SparkConf().setMaster(masterUrl).setAppName(this.getClass.getSimpleName)
    /**
    ...
    val schemaforusers = StructType("UserID::Gender::Age::OccupationID::Zip_Code".split("::").map(column => StructField(column, StringType, true))) //使用Struct方式把Users的数据格式化,即在RDD的基础上增加数据的元数据信息
    val usersRDDRows = usersRDD.map(_.split("::")).map(line => Row(line(0).trim, line(1).trim, line(2).trim, line(3).trim, line(4).trim)) //把我们的每一条数据变成以Row为单位的数据
    ...
    val ratingsRDDRows = ratingsRDD.map(_.split("::")).map(line => Row(line(0).trim, line(1).trim
    ...
    spark.sql("SELECT Gender,Age,count(*) from global_temp.users u join global_temp.ratings as r on u.UserID = r.UserID where MovieID = 1193 group by Gender, Age").show(10)
    ...
    spark.sql("SELECT Gender, Age, count(*) from users u join ratings as r on u.UserID = r.UserID where MovieID = 1193 group by Gender, Age").show(10)
